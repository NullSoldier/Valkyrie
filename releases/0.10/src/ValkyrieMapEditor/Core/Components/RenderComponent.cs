	...
	public class RenderComponent : IEditorComponent
	{
		...
		public void OnSizeChanged(object sender, ScreenResizedEventArgs e)
		{
			...
			if (this.context.SceneProvider.Cameras["camera1"] != null)
			{
				...
				{
					// Move the X origin
					int DisplayedWidth = (MapEditorManager.CurrentMap.MapSize.IntX * MapEditorManager.CurrentMap.TileSize) + (int)camera.Location.IntX;
					...
					// Move the Y origin
					int DisplayedHeight = (MapEditorManager.CurrentMap.MapSize.IntY * MapEditorManager.CurrentMap.TileSize) + (int)camera.Location.IntY;
		...
		public void Draw(SpriteBatch spriteBatch)
		{
			...
			{
				this.context.SceneProvider.DrawCamera("camera1", false);
			}
